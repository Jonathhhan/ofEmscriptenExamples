...
void ofApp::update() {
	videoPlayer.update();
	if (videoPlayer.isFrameNew() && frames.size() > 0) {
		TS_START("measurement1");
		fboCellsCopy.begin();
		videoPlayer.draw(0, 0, fboCellsCopy.getWidth(), fboCellsCopy.getHeight());
		glCopyImageSubData(fboCellsCopy.getTexture().getTextureData().textureID, fboCellsCopy.getTexture().getTextureData().textureTarget, 0, 0, 0, 0, frames[frames.size() - 1].getTextureData().textureID, frames[frames.size() - 1].getTextureData().textureTarget, 0, 0, 0, 0, frames[frames.size() - 1].getWidth(), frames[frames.size() - 1].getHeight(), 1);
		fboCellsCopy.end();
		frames.push_front(frames[frames.size() - 1]);
		if (frames.size() > texture3d.getVolumeDepth() && !bang_iterate.value) {
			...
			for (int x = 0; x < texture3d.getVolumeDepth(); x++) {
				videoShader.setUniformTexture("Tex1", frames[x], 1);
				videoShader.setUniform1i("zCount", x);
				ofClear(255);
				ofDrawRectangle(0, 0, fboCells.getWidth(), fboCells.getHeight());
				glCopyImageSubData(fboCellsCopy.getTexture().getTextureData().textureID, fboCellsCopy.getTexture().getTextureData().textureTarget, 0, 0, 0, 0, texture3d.getTextureData().textureID, texture3d.getTextureData().textureTarget, 0, 0, 0, x, frames[frames.size() - 1].getWidth(), frames[frames.size() - 1].getHeight(), 1);
			}
			videoShader.end();
			fboCellsCopy.end();
		}
	...
	if (bang_iterate.value && videoPlayer.isFrameNew()) {
		float cells[54] = {
		...
		for (int x = 0; x < texture3d.getVolumeDepth(); x++) {
			shaderGameOfLife.setUniformTexture("Tex1", frames[x], 1);
			shaderGameOfLife.setUniform1i("zCount", x);
			ofClear(255);
			ofDrawRectangle(0, 0, fboCells.getWidth(), fboCells.getHeight());
			glCopyImageSubData(fboCellsCopy.getTexture().getTextureData().textureID, fboCellsCopy.getTexture().getTextureData().textureTarget, 0, 0, 0, 0, texture3dCopy.getTextureData().textureID, texture3dCopy.getTextureData().textureTarget, 0, 0, 0, x, frames[frames.size() - 1].getWidth(), frames[frames.size() - 1].getHeight(), 1);
		}
		shaderGameOfLife.end();
		fboCellsCopy.end();
		glCopyImageSubData(texture3dCopy.getTextureData().textureID, texture3dCopy.getTextureData().textureTarget, 0, 0, 0, 0, texture3d.getTextureData().textureID, texture3d.getTextureData().textureTarget, 0, 0, 0, 0, texture3d.getVolumeWidth(), texture3d.getVolumeHeight(), texture3d.getVolumeDepth());
	...
	if (hRadio_pattern.boolean) {
		...
		if (number_gridNumX.value != oldGridNumX || number_gridNumY.value != oldGridNumY || number_gridNumZ.value != oldGridNumZ) {
			texture3d.setup(number_gridNumX.value, number_gridNumY.value, number_gridNumZ.value, ofVec3f(1, 0.5, 1), false);
			texture3d.setRenderSettings(1, 5, 0.5, 0.1);
			texture3dCopy.setup(number_gridNumX.value, number_gridNumY.value, number_gridNumZ.value, ofVec3f(1, 0.5, 1), false);
			fboCells.allocate(number_gridNumX.value, number_gridNumY.value);
			fboCellsCopy.allocate(number_gridNumX.value, number_gridNumY.value);
			shaderRaster.begin();
			...
			oldGridNumZ = number_gridNumZ.value;
			frames.clear();
			while (frames.size() < number_gridNumZ.value) {
				ofTexture tex;
				tex.allocate(number_gridNumX.value, number_gridNumY.value, GL_RGB8);
...
void ofApp::draw() {
	...
	TS_START("measurement3");
	texture3d.drawVolume(0, 75, 0, 300, 0);
	TS_STOP("measurement3");
	easyCam.end();
