var LibraryHTML5Video = {
					...
					context.strokeStyle = "green";
  					faces.forEach((face) => {
						...
						for(let i = 0; i < TRIANGULATION.length; i+=3) {
							...
							context.beginPath();
							context.moveTo(keypoints[i1][0], keypoints[i1][1]);
							context.lineTo(keypoints[i2][0], keypoints[i2][1]);
							context.lineTo(keypoints[i3][0], keypoints[i3][1]);
							context.closePath();
							context.stroke();
						}
					});
   ...
   html5video_player_create: function(){	
	...
	faceLandmarksDetection.createDetector(model, detectorConfig).then(dect => {
		detector = dect;
	});
	TRIANGULATION = [
