        ...
        update: function(updatePixels, video, context, dstPixels){
        	if((updatePixels || video.pixelFormat!="RGBA") && video.width!=0 && video.height!=0 && dstPixels!=0){
        		try {
	            	...
	            	}else if(video.pixelFormat=="RGB" && typeof segmenter != "undefined"){
	            		segmenter.segmentPeople(video).then(people => {
					const foregroundColor = {r: 0, g: 0, b: 0, a: 0};
					const backgroundColor = {r: 100, g: 0, b: 0, a: 255};
					const drawContour = true;
					const foregroundThreshold = 0.1;
					bodySegmentation.toBinaryMask(people, foregroundColor, backgroundColor, drawContour, foregroundThreshold).then(coloredPartImage => {
						const opacity = 0.9;
						...
						bodySegmentation.drawMask(canv, video, coloredPartImage, opacity, maskBlurAmount, flipHorizontal).then(()=>{
							imageData = cont.getImageData(0,0,video.width,video.height);
							array = Module.HEAPU8.subarray(dstPixels, dstPixels+(video.width*video.height*3));	
		            				for(var i=0, j=0; i<array.length; ){
		            					array[i++] = imageData.data[j++];
		            					array[i++] = imageData.data[j++];
		            					array[i++] = imageData.data[j++];
		            					++j;
		            				}	
							GLctx.bindTexture(GLctx.TEXTURE_2D, GL.textures[video.textureId]);
                					GLctx.texImage2D(GLctx.TEXTURE_2D, 0, GLctx.RGB, video.width, video.height, 0, GLctx.RGB, GLctx.UNSIGNED_BYTE, imageData);
