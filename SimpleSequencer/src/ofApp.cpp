...
void ofApp::setup()  
{
    ...
    const int sampleRate = ofSoundStream().getSampleRate(); // audio sample rate
    const int ticksPerBuffer = 2; // used to compute the audio buffer len: tpb * blocksize (always 64)
