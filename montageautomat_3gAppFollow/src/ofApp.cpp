...
void ofApp::update() {
	...
	int currentSubtitle = selectedSubtitle + numberOfSubtitles;
	int currentVideoPlayer = numberOfVideoPlayer;
	if (!mapSubVectorCopy.empty() && subVector[numberOfVideoPlayer][currentSubtitle] -> getEndTime() + 50 < videoPlayerVector[numberOfVideoPlayer] -> getPosition() * videoPlayerVector[numberOfVideoPlayer] -> getDuration() * 1000 && mapSubVectorCopy.size() > 0) {
		std::multimap<double, std::tuple<int, int, int>> multimapWeightSub;
		for (auto &element : mapSubVectorCopy) {
			if (element.first.first != numberOfVideoPlayer || element.first.second != selectedSubtitle) {
				if (!bCustomWords || customWords.empty()) {
					multimapWeightSub.insert(std::make_pair(mapSubVectorCopy[{numberOfVideoPlayer, selectedSubtitle + 1}].first.dist_cosine_optimized(element.second.first), std::make_tuple(element.first.first, element.first.second, element.second.second)));
				}
				else {
		...
		mapSubVectorCopy.erase({numberOfVideoPlayer, selectedSubtitle + 1});
		...
		if ((selectedSubtitle > 0 && currentSubtitle != selectedSubtitle - numberOfSubtitles) || (selectedSubtitle > 0 && currentVideoPlayer != numberOfVideoPlayer)) {
			int subtitle = selectedSubtitle - 1;
			videoPlayerVector[numberOfVideoPlayer] -> setPosition((subVector[numberOfVideoPlayer][subtitle] -> getEndTime() + 50) / videoPlayerVector[numberOfVideoPlayer] -> getDuration() / 1000);
		}
		else if (selectedSubtitle == 0) {
			videoPlayerVector[numberOfVideoPlayer] -> setPosition(0);
		}
		if (currentVideoPlayer != numberOfVideoPlayer) {
			videoPlayerVector[currentVideoPlayer] -> stop();
			videoPlayerVector[numberOfVideoPlayer] -> play();
		}
		std::vector<std::string> words;
		for (int i = 0; i <= numberOfSubtitles; i++) {
