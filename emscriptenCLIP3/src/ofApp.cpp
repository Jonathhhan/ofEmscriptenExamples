...
std::string textModelStatus;
bool textModelIsLoaded;

void textModelLoaded(){    
	textModelStatus = "Text CLIP model loaded. Type something.";
	textModelIsLoaded = true;
}

//--------------------------------------------------------------
EMSCRIPTEN_BINDINGS(Module){
	emscripten::function("textModelLoaded", &textModelLoaded);
}

//--------------------------------------------------------------
void ofApp::setup(){
	textModelIsLoaded = false;
	ofSetBackgroundColor(100, 100, 100);
	textModelStatus = "Text CLIP model loading... (be patient).";
...
//--------------------------------------------------------------
void ofApp::draw(){
	ofDrawBitmapString(textModelStatus, 20, 20);
