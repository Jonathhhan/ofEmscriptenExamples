...
void ofApp::update() {
	...
	if (videoPlayer.isFrameNew()) {
		...
		for (int x = 0; x < texture3d.getVolumeDepth(); x++) {
			...
			videoShader.setUniformTexture("Tex1", videoPlayer.getTexture(), 1);
	...
	if (bang_iterate.value) {
		...
		for (int x = 0; x < texture3d.getVolumeDepth(); x++) {
			...
			shaderGameOfLife.setUniformTexture("Tex1", videoPlayer.getTexture(), 1);
			shaderGameOfLife.setUniform3f("resolution", texture3d.getVolumeWidth(), texture3d.getVolumeHeight(), texture3d.getVolumeDepth());
			shaderGameOfLife.setUniform2f("res", videoPlayer.getTexture().getWidth(), videoPlayer.getTexture().getHeight());
