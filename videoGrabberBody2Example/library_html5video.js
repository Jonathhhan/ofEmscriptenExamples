        	...
        	if((updatePixels || video.pixelFormat!="RGBA") && video.width!=0 && video.height!=0 && dstPixels!=0){
        		try {
	            	if (video.pixelFormat=="RGBA"){
	            		...
	            		segmenter.segmentPeople(video, {multiSegmentation: false, segmentBodyParts: true}).then(people => {
					bodySegmentation.toColoredMask(people, bodySegmentation.bodyPixMaskValueToRainbowColor, {r: 255, g: 255, b: 255, a: 255}).then(coloredPartImage => {
						const opacity = 0.9;
						...
						bodySegmentation.drawPixelatedMask(context.canvas, video, coloredPartImage, opacity, maskBlurAmount,
    flipHorizontal, pixelCellWidth).then(()=>{
